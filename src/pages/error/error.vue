<template>
  <div class="bg-gray-100 h-screen justify-center">
    ...
    <span class="text-gray-500 text-6xl block"><span>{{errorment}}</span></span>
    <span class="text-gray-500 text-xl">페이지 오류가 발생했습니다</span>
    </div>
    </center>
    <br>
    <center class="mt-6">
    <a href="/Catchvegan" class="text-gray-500 font-mono text-xl bg-gray-200 p-3 rounded-md hover:shadow-md">홈페이지로 이동</a>
    </center>
  </div>
</template>
...
export default({
  ...
  setup(){
    ...
    const check = () => {
        if(token == null){
          console.log(errorment);
          errorment = '4 0 4';
        }else{
          console.log(errorment.value);
        }
      }
    check();
  }
